...
RESULTS_ROOT = ROOT / "results" / "decoder_only_qwen_metrics"
...
TASKS = {
    "python2java": {
        "source": "python",
        "target": "java",
        "output_dir": OUTPUT_ROOT / "qwen25_coder_05b_decoder_only_ultrafast_python2java",
    ...
    "java2python": {
        "source": "java",
        "target": "python",
        "output_dir": OUTPUT_ROOT / "qwen25_coder_05b_decoder_only_ultrafast_java2python",
...
def evaluate_task(name, batch_size, workers, timeout, skip_compile, skip_exec):
    ...
    run_meta = {
        ...
        "source": task["source"],
        "target": task["target"],
        "model": "Qwen2.5-Coder-0.5B-Instruct decoder-only QLoRA ultrafast",
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate decoder-only Qwen AVATAR predictions with resumable logs.")
    ...
    task_batch_size = args.batch_size or TASKS[args.task]["batch_size"]
