from constants import BASE_TRAIN_CONFIG, OUTPUT_ROOT, SEQ2SEQ_MODEL
...
def build_dataset(src_texts, tgt_texts, token_to_id, max_source_length, max_target_length, batch_size, shuffle):
...
def main():
    ...
    if not args.eval_only:
        ...
        patience = 0
        steps_per_epoch = (len(train_src) + args.batch_size - 1) // args.batch_size
        for epoch in range(start_epoch + 1, args.epochs + 1):
            ...
            epoch_result = {"epoch": epoch, "train_loss": round(sum(losses) / max(len(losses), 1), 4), "valid_bleu": round(valid_bleu, 2)}
